...
export default function Navigation() {
  ...
  return (
    <header className='sticky top-3 flex items-center justify-between z-50 p-2.5 rounded-xl'>
      <Link href={'/'} className='flex items-center gap-4'>
        <Image src={Images.SavannaLogo} alt='savanna logo' className='w-44 min-h-10' />
      </Link>
      <nav className='relative flex items-center justify-center shadow gap-3 bg-white p-1 rounded-full'>
        ...
        {!SignedIn ? 
          <>
            ...
            <Link href={'/auth/sign-in'}>
              <Button className='py-4.5 px-5'>Log In</Button>
            </Link>
          </> :
